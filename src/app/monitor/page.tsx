...
export default function MonitorPage() {
  ...
  // Satellite date state — starts with today, controlled by slider in RightPanel
  const [satDate, setSatDate] = useState(to);

  const { events, loading: eventsLoading, error: eventsError } = useEvents({ from, to });

  const [selectedEventId, setSelectedEventId] = useState<string | null>(null);
  const { pack, loading: packLoading, error: packError } = useEventPack(selectedEventId);
    ...
    <div className="flex h-[calc(100vh-64px)] w-full bg-slate-50/70">
      ...
      <div className="flex-1 relative overflow-hidden">
        <WorldMonitorMap
          ...
          satDate={satDate}           // Controlled by state
        />
      ...
      {/* Right: Detail panel (contains the satellite date slider) */}
      <RightPanel
        selectedEventId={selectedEventId}
        ...
        packError={packError}
        satDate={satDate}
        setSatDate={setSatDate}     // Allows slider to update date
      />
    </div>
  );
}
